/* ==========================================================================
   codebase-blueprint-ai — renderer
   ---------------------------------------------------------------------------
   This file is FIXED. An LLM authoring a blueprint never edits it; it only
   writes a codeviz.json that conforms to schema/codeviz.schema.json.

   Everything here is deterministic: no Math.random, no Date-seeded values.
   The same JSON always produces the same drawing.
   ========================================================================== */

import * as THREE from 'three';

window.__codevizStarted = true;   // tells the shell's watchdog to stand down
...
function readData() {
  const tag = document.getElementById('codeviz-data');
  if (!tag) throw new Error('no #codeviz-data block in the document');
  const raw = tag.textContent.trim();
  if (!raw || raw.indexOf('__CODEVIZ_DATA__') !== -1) {
    throw new Error('the data block is still the unfilled placeholder');
  }
  let d;
  try { d = JSON.parse(raw); }
  catch (e) { throw new Error('the data block is not valid JSON — ' + e.message); }
  if (d.codeviz !== '1.0') throw new Error('unsupported codeviz version: ' + d.codeviz);
  for (const k of ['meta', 'groups', 'nodes', 'edges', 'narrative']) {
    if (!d[k]) throw new Error('missing required key: ' + k);
  }
  return d;
}
...
try {
  ...
  noDataEl.querySelector('.box').appendChild(e);
  throw err;
}
...
function buildNode(node) {
  ...
  scene.add(grp);
  const topY = grp.position.y + hy + 0.12;
...
(function narrative() {
  ...
  const bodyEl = document.getElementById('narrbody');
...
function paintHighlight() {
  ...
  document.querySelectorAll('#sidebar .nrow').forEach(r => {
  ...
  document.querySelectorAll('#narrbody .nref').forEach(r => {
    r.classList.toggle('hot', hotNodes.has(r.dataset.node));
  });
}
...
stage.addEventListener('pointerup', ev => {
  ...
  if (!dragged) {
    const h = pick(ev);
    if (h?.type === 'node') { selected = selected === h.id ? null : h.id; focusNode(selected); }
...
window.addEventListener('keydown', ev => {
  ...
  else if (k === 'escape') { selected = null; setHover(null, null); paintHighlight(); }
});
...
const labelSize = new Map();   // measured once, text never changes
...
function updateLabels() {
  ...
  if (!labelMeasured) {
    for (const [id, el] of labelEls) labelSize.set(id, [el.offsetWidth || 40, el.offsetHeight || 10]);
    labelMeasured = [...labelSize.values()].some(s => s[0] > 0);
  }
  ...
  for (const id of pass) {
    ...
    const rect = [x - lw / 2 - 2, y - lh / 2 - 1, x + lw / 2 + 2, y + lh / 2 + 1];
    if (!forced) {
      let clash = false;
...
function tick() {
  ...
  window.__codevizFrames = (window.__codevizFrames || 0) + 1;
}
...
// small surface for automated checks / console poking
window.CODEVIZ = {
  data: DATA, nodeViews, edgeViews, camera, cam,
  focusNode, fitAll,
